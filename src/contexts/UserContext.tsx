...
import { initializeStreakData, updateStreakOnActivity } from '../utils/streakUtils';
...
export const UserProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        const loadProfile = async () => {
            console.log('[UserContext] 📥 loadProfile called at', new Date().toISOString());
            setLoading(true);
            try {
                // 1. Check local storage for immediate data
                let localUser: UserProfile | null = null;
                const storedUser = localStorage.getItem('palante_user');
                if (storedUser) {
                    try {
                        ...
                        if (localUser) {
                            localUser = {
                                ...
                                quoteIntensity: (Number(localUser.quoteIntensity) || Number((localUser as any).tier) || 2) as any,
                ...
                if (authUser) {
                    try {
                        ...
                        if (cloudProfile) {
                            ...
                            localStorage.setItem('palante_user', JSON.stringify(cloudProfile));
                            // Sync Widgets
                            ...
                            localStorage.setItem('palante_user', JSON.stringify(guestUser));
                            // Sync Widgets
    ...
    const updateProfile = useCallback(async (updateInput: UserProfile | ((prev: UserProfile | null) => UserProfile)) => {
        console.log('[UserContext] 💾 updateProfile called at', new Date().toISOString());
        ...
        localStorage.setItem('palante_user', JSON.stringify(newUser));
    ...
    const logActivity = useCallback(async (type: ActivityType) => {
        ...
        await updateProfile(prevUser => {
            ...
            const currentStreakData = prevUser.streakData || initializeStreakData();
            const updatedStreakData = updateStreakOnActivity(currentStreakData, type);
    ...
    const refreshProfile = useCallback(async () => {
        ...
        if (profile) {
            setUser(profile);
            localStorage.setItem('palante_user', JSON.stringify(profile));
        }
    }, [authUser]);

...
//
// Conditional on AI_FEATURES_ENABLED rather than a single fixed list: while that
// flag is off (see featureFlags.ts — not shipping with an AI coach at this time),
// every message is written from a fallback template instead of generated live, so
// claiming "AI-written" here would be selling something this build does not do.

export const FEATURES = AI_FEATURES_ENABLED ? [
...
/**
 * AI disclosure text: surfaced in the UI per Apple Guideline 2.5 (AI-generated
 * content must be disclosed). That requirement only applies to content that is
 * actually AI-generated — with AI_FEATURES_ENABLED off, nothing sold on this
 * screen is, so there is nothing to disclose. `null`, not an empty string, so
 * the render site can tell "nothing to show" apart from "empty copy, likely a
 * bug" — see its usage below.
 */
export const AI_DISCLOSURE: string | null = AI_FEATURES_ENABLED
    ? 'Affirmations, coaching messages, and reflections are generated by AI, personalized to your entries.'
    : null;

...
export const ReorderModal: React.FC<ReorderModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-start justify-center animate-fade-in p-4 pt-20 overflow-hidden">
            ...
            <div className={`relative w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300 ${isDarkMode ? 'bg-warm-gray-green border border-white/10' : 'bg-[#fcf8f2] border border-sage/10'
                }`}>
                ...
                <div className="p-4 bg-black/5">
                    <button
                        onClick={handleSave}
                        className={`w-full py-3.5 rounded-2xl font-bold uppercase tracking-[0.2em] text-[10px] flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-xl ${isDarkMode ? 'bg-pale-gold text-warm-gray-green shadow-pale-gold/10' : 'bg-sage text-white shadow-sage/20'
                            }`}
                    >

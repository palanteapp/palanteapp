...
import { Settings, Check, X, ShieldCheck, Sparkles, Zap, Volume2, Heart, Leaf } from 'lucide-react';
...
const SettingRow = ({
    ...
    >
        ...
        <div className="flex-1">
            ...
            <p className={`text-[9px] leading-relaxed transition-opacity mt-0.5 ${options[id] ? 'opacity-90' : 'opacity-40'} ${isDarkMode ? 'text-white/80' : 'text-warm-gray-green'}`}>
                {description}
            </p>
        </div>
...
export const EnhancementSettings: React.FC<EnhancementSettingsProps> = ({ isOpen, onClose, isDarkMode, onUpdate, exclude = [] }) => {
    ...
    useEffect(() => {
        localStorage.setItem('palante_enhancements', JSON.stringify(options));
    ...
    return (
        ...
        >
            <div className={`w-full overflow-hidden ${isDarkMode ? 'bg-warm-gray-green' : 'bg-white'}`}>
                {/* Header */}
                ...
                {/* Footer - Minimalist */}
                <div className={`mt-2 p-4 bg-black/5 flex items-start gap-3 ${isDarkMode ? 'text-white/30' : 'text-warm-gray-green/40'}`}>

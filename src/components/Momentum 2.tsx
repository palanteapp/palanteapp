...
export const Momentum: React.FC<MomentumProps> = ({
    ...
    // Trigger Evening Reflection after 6 PM
    useEffect(() => {
        const checkEveningTrigger = () => {
            const now = new Date();
            const hour = now.getHours();
            const today = now.toISOString().split('T')[0];

            const hasReflectedToday = user.journalEntries?.some(e => e.date === today);

            // Check if we should trigger: 6pm+, no reflection yet, active goals existing, and modal not already open
            if (hour >= 18 && !hasReflectedToday && dailyFocuses.length > 0 && !showEveningReflection) {
                // We use a small timeout to avoid immediate popup on mount if criteria met
                const t = setTimeout(() => setShowEveningReflection(true), 2000);
                return () => clearTimeout(t);
            }
        };

        // Run immediately
        const cleanup = checkEveningTrigger();

        // Run every minute
        const interval = setInterval(checkEveningTrigger, 60000);

        return () => {
            clearInterval(interval);
            if (cleanup) cleanup();
        };
    }, [user.journalEntries, dailyFocuses, showEveningReflection]);

    const handleSaveReflection = (entry: JournalEntry) => {
        const updatedEntries = [...(user.journalEntries || []), entry];
        const uniqueEntries = updatedEntries.filter((e, i, self) =>
            i === self.findIndex((t) => t.date === e.date)
        );
    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-6 pt-6 pb-40 animate-fade-in">
            ...
            <div className="mb-10">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                        ...
                        className={`text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border transition-all ${isDarkMode
            ...
            <SlideUpModal
                ...
                isDarkMode={isDarkMode}
            >
                <div className={`p-6 w-full max-w-sm mx-auto ${isDarkMode ? 'text-white' : 'text-warm-gray-green'}`}>
                        ...
                        {[
                            ...
                            <div key={i} className={`flex items-start gap-4 p-4 rounded-xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/60 border-sage/20'}`}>
                                ...
                                <div>
                                    ...
                                    <div className={`text-xs ${isDarkMode ? 'text-white/50' : 'text-warm-gray-green/50'}`}>{item.desc}</div>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <EveningCheckIn
                isOpen={showEveningReflection}
                onClose={() => setShowEveningReflection(false)}
                onSaveReflection={handleSaveReflection}
                completedGoals={dailyFocuses.filter(f => f.isCompleted)}
                totalGoals={dailyFocuses.length}

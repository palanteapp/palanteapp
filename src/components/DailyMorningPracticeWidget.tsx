...
export const DailyMorningPracticeWidget: React.FC<DailyMorningPracticeProps> = ({ onComplete, onRefresh, isDarkMode, existingPriming, userName, hideEnergyCheckIn, onFinish }) => {
    ...
    const renderIntro = () => (
        <div className="flex flex-col items-center text-center py-8 animate-fade-in">
            ...
            <p className={`text-base max-w-xs mb-8 ${textSecondary}`}>
                Before we tackle the to-do list, let’s orient your mind towards abundance and power.
            </p>
            <button
                onClick={handleNext}
                className={`px-8 py-3 rounded-full font-medium transition-all hover:scale-105 ${isDarkMode ? 'bg-pale-gold text-warm-gray-green' : 'bg-sage text-white'}`}
            >
                Start Practice
            </button>
        </div>
        ...
        return (
            <div className="w-full py-8 text-center animate-fade-in min-h-[300px] flex flex-col justify-center">
                {isGenerating ? (
                    ...
                    <>
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg ${isDarkMode ? 'bg-pale-gold text-warm-gray-green' : 'bg-sage text-white'}`}>
                        ...
                        <button
                            onClick={handleNext}
                            className={`mt-12 px-10 py-3 rounded-full font-medium shadow-lg transition-all hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-pale-gold text-warm-gray-green' : 'bg-sage text-white'}`}
                        >
                            Embrace This
                        </button>
                    </>
    ...
    const renderSummary = () => (
        <div className="w-full py-4 animate-fade-in text-center">
            ...
            <div className="flex flex-col gap-3">
                <button
                    onClick={onFinish}
                    className={`w-full py-4 rounded-xl text-lg font-bold tracking-wide transition-all active:scale-95 shadow-lg ${isDarkMode ? 'bg-pale-gold text-warm-gray-green' : 'bg-sage text-white'}`}
                >
                    Return to Dashboard
                </button>
                <button

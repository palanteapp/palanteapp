...
export const UpdatePasswordModal: React.FC<UpdatePasswordModalProps> = ({ isOpen, onClose, isDarkMode }) => {
    ...
    const inputBg = isDarkMode ? 'bg-white/5 border-white/10 focus:border-pale-gold text-white' : 'bg-white border-sage/10 focus:border-sage text-warm-gray-green';

    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 animate-fade-in">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />

            <div className={`relative w-full max-w-md max-h-[90vh] overflow-y-auto p-6 rounded-2xl shadow-xl animate-slide-up border ${isDarkMode ? 'bg-warm-gray-green border-white/10' : 'bg-white border-sage/20'}`}>
                <button
                    onClick={onClose}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button
                            ...
                            className={`w-full py-3 rounded-xl font-medium transition-all flex items-center justify-center gap-2 ${loading || !password || !confirmPassword
                                ...
                                : isDarkMode ? 'bg-pale-gold text-warm-gray-green hover:bg-white' : 'bg-sage text-white hover:bg-sage/90'
                                }`}
                        >

...
export const MilestoneModal: React.FC<MilestoneModalProps> = ({ milestone, onClose, isDarkMode }) => {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#3A1700]/70 backdrop-blur-sm animate-fadeIn">
            <div className={`relative max-w-md w-full max-h-[90vh] overflow-y-auto p-8 rounded-3xl shadow-2xl animate -scaleIn ${isDarkMode ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-gray-200'
                ...
                <div className="text-center">
                    ...
                    <button
                        onClick={onClose}
                        className={`w-full py-4 rounded-full font-bold transition-all transform hover:scale-[1.02] ${isDarkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-[#3A1700] text-white hover:bg-gray-800'
                            }`}
                    >
                        Keep Going!
                    </button>

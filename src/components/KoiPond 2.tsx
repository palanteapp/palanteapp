...
export const KoiPond: React.FC<KoiPondProps> = ({ isDarkMode, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (window.DeviceOrientationEvent && (window as any).DeviceMotionEvent) {
    ...
    const bgClass = 'bg-warm-gray-green'; // Consistent with app aesthetic
    ...
    */
    ...
    const toggleMute = useCallback((e: React.MouseEvent) => {
        ...
        if (audio1Ref.current) {
            ...
            else if (!(audio1Ref.current as any)._fadeInterval) {
        ...
        if (audio2Ref.current) {
            ...
            else if (!(audio2Ref.current as any)._fadeInterval) {
                // If not currently fading, restore target volume
    ...
    return (
        ...
        <div
            className="fixed inset-0 z-50 bg-black transition-colors duration-[3000ms] overflow-hidden cursor-pointer"
        ...
        >
            ...
            >
                <div className="relative">
                    ...
                    <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-48 rounded-2xl border backdrop-blur-xl transition-all duration-300 origin-bottom overflow-hidden ${isSettingsOpen
                        ...
                        } ${isDarkMode ? 'bg-warm-gray-green/90 border-white/10' : 'bg-white/90 border-sage/10'}`}>
                        <div className="p-2 space-y-1 text-left">
                            {[
                                ...
                                { label: 'Baby Lotus', state: showBabyLotus, setter: setShowBabyLotus },
                                {
                                    label: 'Nature Particles', state: showParticles, setter: (val: boolean) => {
                                        ...
                                        if (saved) {
                                            ...
                                            localStorage.setItem('palante_enhancements', JSON.stringify(parsed));
                                ...
                                >
                                    <span className={`text-xs font-medium ${isDarkMode ? 'text-white' : 'text-warm-gray-green'}`}>
                                        {item.label}
                                    </span>
            ...
            <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-40 text-center pointer-events-none transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <p className={`text-sm mt-1 mb-3 ${isDarkMode ? 'text-white/60' : 'text-warm-gray-green/60'}`}>
                    Observe and relax
                </p>
            </div>
        </div>

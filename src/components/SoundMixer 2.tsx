...
import { VolumeX, Cloud, Wind, Waves, Trees, Droplets, Zap, Radio, Moon, Sun, Music, Speaker, Bird, Save, Plus, X, Coffee, Sparkles, HelpCircle, Info } from 'lucide-react';
...
const getAudioContext = () => {
    ...
    if (!(window as any)._palanteAudioContext) {
        (window as any)._palanteAudioContext = new AudioContextClass();
    }
    return (window as any)._palanteAudioContext as AudioContext;
};
...
export const SoundMixer: React.FC<SoundMixerProps> = ({ isDarkMode: _isDarkMode, isVisible, onClose, user, onSaveMix, onDeleteMix, source }) => {
    ...
    const [showHelp, setShowHelp] = useState(() => {
        const hasSeenHelp = localStorage.getItem('palante_soundmixer_help_seen');
        return !hasSeenHelp;
    });
        ...
        const updateWakeLock = async () => {
            if (activeSounds.size > 0) {
                try {
                    ...
                    console.log('🔒 SoundMixer: Wake Lock acquired');
                } catch (e) {
    ...
    useEffect(() => {
        ...
        const handleRestartSounds = () => {
            console.log('🎵 SoundMixer: Restarting all active sounds');
        ...
        const handleLoadPreset = (event: any) => {
            const presetId = event.detail?.preset;
            if (presetId && MEDITATION_PRESETS[presetId]) {
                console.log(`🎵 SoundMixer: Loading preset ${presetId}`);
                ...
                setTimeout(() => {
                    ...
                    Object.entries(preset.volumes).forEach(([id, vol]) => {
                        ...
                        if (soundData) {
                            ...
                            console.log(`  ▶️ Playing ${id} at volume ${vol}`);
                            newActive.add(id);
                            newVols[id] = vol;
                            audio.play(vol);
                        } else {
                    ...
                    setVolumes(newVols);
                    console.log(`✅ Preset loaded. Active:`, Array.from(newActive));
                }, 100);
            } else {
        ...
        const handleToggleSound = (event: any) => {
        ...
        window.addEventListener('palante-load-preset', handleLoadPreset);
        window.addEventListener('palante-toggle-sound', handleToggleSound);
        window.addEventListener('palante-set-volume', handleSetVolume);
        ...
        return () => {
            ...
            window.removeEventListener('palante-load-preset', handleLoadPreset);
            window.removeEventListener('palante-toggle-sound', handleToggleSound);
            window.removeEventListener('palante-set-volume', handleSetVolume);
        };
    ...
    return (
        <div
            className="fixed inset-0 z-[60] flex flex-col pt-6 animate-fade-in backdrop-blur-xl text-white"
            style={{ backgroundColor: '#6F7B6D' }}
        >

            {/* TOP BAR: Header & Global Controls */}
            {/* TOP BAR: Header & Global Controls */}
            <div className={`px-4 md:px-8 py-4 md:py-6 flex flex-col md:flex-row items-center justify-between z-10 border-b border-white/5 bg-black/5 backdrop-blur-md gap-4`}>
                ...
                <div className="flex bg-black/40 rounded-2xl p-1 border border-white/10 items-center shadow-inner w-full md:w-auto overflow-x-auto no-scrollbar">
                    <button
            ...
            {/* HELP OVERLAY */}
            {showHelp && (
                <div className="absolute inset-0 z-50 bg-warm-gray-green/95 backdrop-blur-sm flex items-center justify-center p-8 animate-fade-in">
                    <div className="max-w-2xl bg-sage/95 rounded-3xl p-8 relative border border-white/20 shadow-2xl">
                        <button
                            onClick={() => {
                                setShowHelp(false);
                                localStorage.setItem('palante_soundmixer_help_seen', 'true');
                            }}
                        ...
                        <button
                            ...
                            className="mt-6 w-full py-3 rounded-xl bg-pale-gold text-warm-gray-green font-bold uppercase tracking-widest hover:bg-white transition-colors"
                        >
                            Got It!
                        </button>
                    </div>
                        ...
                        {activeSoundList.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 px-4">
                                        ...
                                        {user?.savedMixes?.map(mix => (
                                            <div key={mix.id} className="group relative h-48 rounded-[2.5rem] overflow-hidden border border-white/5 bg-white/5 transition-all duration-700 hover:scale-[1.02] active:scale-98 shadow-2xl">
                                                <button
                                                    onClick={() => loadRecipe({ id: mix.id, name: mix.name, volumes: mix.volumes, color: 'from-pale-gold/20 to-sage/20' } as any)}
                                                    className="absolute inset-0 w-full h-full text-left p-8 flex flex-col justify-end z-10"
                                                >
                                                    <div className="absolute inset-0 bg-gradient-to-br from-pale-gold/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                                                ...
                                                <button
                                                    ...
                                                    className="absolute top-6 right-6 z-20 p-2.5 rounded-2xl bg-black/40 hover:bg-red-500/40 text-white/20 hover:text-white transition-all opacity-0 group-hover:opacity-100 backdrop-blur-md border border-white/5"
                                                >
                                                    <X size={12} />
                                                </button>
                                            </div>
                                        ...
                                        {RECIPES.map(recipe => (
                                            <button
                                                ...
                                                className="group relative h-40 md:h-48 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-white/5 bg-black/20 transition-all duration-700 hover:scale-[1.02] active:scale-98 text-left p-6 md:p-8 flex flex-col justify-end shadow-2xl"
                                            >
                                        ...
                                        {categories.map(cat => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className="group flex flex-col items-center gap-4 md:gap-6 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] bg-black/20 border border-white/5 hover:bg-white/5 hover:border-white/10 hover:scale-[1.05] active:scale-95 transition-all duration-500 shadow-xl"
                                                >
                            ...
                            activeSoundList.map(sound => (
                                <div key={sound.id} className="flex-shrink-0 w-20 md:w-32 h-[55vh] flex flex-col items-center justify-end group relative animate-slide-up-fade">
                                    ...
                                    <div className="relative w-12 md:w-20 h-full bg-black/40 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden flex flex-col justify-end group-hover:bg-black/50 transition-all duration-500 backdrop-blur-3xl border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                                ...
                                <div key={cat} id={`category-${cat}`} className="scroll-mt-24">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                        {SOUNDS.filter(s => s.category === cat).map(sound => {
                                            ...
                                            const currentVolume = volumes[sound.id] || 0.5;
                                            return (
                                                <div
                                                    key={sound.id}
                                                    className={`
                                                        ...
                                                        ${isActive
                                                            ...
                                                            : 'bg-black/20 border-white/5 hover:bg-white/5 hover:border-white/10 text-white'}
                                                    `}
                                                >
                                                    <button
                                                        onClick={() => toggleSound(sound.id)}
                        ...
                        {isSavingMix ? (
                            <div className="flex-1 bg-white/10 backdrop-blur-xl rounded-2xl p-2 border border-white/20 flex gap-2 animate-fade-in shadow-2xl">
                                ...
                                <button
                                    ...
                                    className="px-6 py-3 rounded-xl bg-pale-gold text-warm-gray-green font-bold uppercase tracking-widest hover:bg-white transition-colors"
                                >
                                    Save
                                </button>
                                <button

...
export const GoalTracker: React.FC<GoalTrackerProps> = ({
    ...
    const textSecondary = isDarkMode ? 'text-white/50' : 'text-warm-gray-green/60';
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${isDarkMode ? 'bg-white/10' : 'bg-sage/10'}`}>
                        <Target size={20} className={isDarkMode ? 'text-pale-gold' : 'text-sage'} />
                    </div>
                ...
                {canAddMore && (
                    <button
                        ...
                        className={`p-2 rounded-full transition-colors ${isDarkMode ? 'hover:bg-white/10 text-white/60' : 'hover:bg-sage/10 text-warm-gray-green/60'}`}
                    >
                        <Plus size={20} />
                    </button>
                )}
                ...
                <div className="mb-8 space-y-4 animate-fade-in">
                    <input
                        ...
                        className={`w-full px-6 py-4 rounded-xl font-body outline-none border transition-all ${isDarkMode
                                ...
                                : 'bg-white border-sage/20 focus:border-sage text-warm-gray-green placeholder-warm-gray-green/30'
                            }`}
                    />
                    <button
                            ...
                            >
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    {todayCheckIn ? (
                                        ...
                                        <button
                                            onClick={() => onCheckIn(goal.id, 1)}
                                            className={`p-2 rounded-full transition-all hover:scale-110 ${isDarkMode ? 'hover:bg-white/10 text-white/40' : 'hover:bg-sage/10 text-warm-gray-green/40'
                                                }`}
                                        >

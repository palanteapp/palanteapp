...
export const CoachSettingsModal: React.FC<CoachSettingsModalProps> = ({
    ...
    return createPortal(
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 animate-fade-in">
            ...
            {/* Modal */}
            <div
                className={`relative max-w-md w-full max-h-[90vh] overflow-y-auto p-8 rounded-[2.5rem] shadow-spa-lg transition-all duration-500 ${isDarkMode ? 'bg-warm-gray-green/95 border border-white/10' : 'bg-white/95 border border-sage/20'}`}
                style={{
            ...
            >
                ...
                <div className="mb-8 space-y-6">
                    <label className="flex items-center justify-between cursor-pointer">
                        <span className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-warm-gray-green'}`}>
                            Enable Nudges
                        </span>
                        <button
                ...
                {localSettings.nudgeEnabled && (
                    <div className="space-y-3 mb-8">
                        <label className={`text-sm font-medium uppercase tracking-wider opacity-80 ${isDarkMode ? 'text-white' : 'text-warm-gray-green'}`}>
                            Nudge Frequency
                        </label>

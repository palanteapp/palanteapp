...
import { analyzeUserBehavior, generateInterventions } from '../utils/aiCoach';
...
export const useAppProcess = ({
    ...
    const aiAnalysisDateRef = useRef<string | null>(null);
    ...
    const checkAndGenerateWeeklyReport = useCallback(() => {
        ...
        if (shouldGenerateWeeklyReport()) {
            ...
            if (!hasReportForThisWeek) {
                ...
                console.log('[useAppProcess] 📊 Calling updateProfile for weekly report');
    ...
    useEffect(() => {
        console.log('[useAppProcess] 📊 Weekly report check at', new Date().toISOString());
    ...
    // 2. AI Coach Analysis
    const analyzeAndGenerateInterventions = useCallback(() => {
        if (!user) return;
        ...
        console.log('[useAppProcess] 🤖 Calling updateProfile for AI coach analysis');
    ...
    /*
    useEffect(() => {
        console.log('[useAppProcess] 🤖 AI coach check at', new Date().toISOString());
        if (!user) return;
    ...
    */
    ...
    useEffect(() => {
        const handleReorder = (e: any) => {
            ...
            console.log('[useAppProcess] 🎯 Calling updateProfile for goal reorder');
        ...
        const handleEdit = (e: any) => {
            ...
            console.log('[useAppProcess] ✏️ Calling updateProfile for goal edit');
            updateProfile(updatedUser);
        };

        window.addEventListener('reorder-focus', handleReorder);
        window.addEventListener('edit-focus', handleEdit);
        return () => {
            window.removeEventListener('reorder-focus', handleReorder);
            window.removeEventListener('edit-focus', handleEdit);
        };
    }, [user, updateProfile]);
    ...
    useEffect(() => {
        const checkMidnight = () => {
            const lastResetDate = localStorage.getItem('palante_last_reset_date');
            const today = new Date().toISOString().split('T')[0];

            if (lastResetDate && lastResetDate !== today) {
                // Logic for reset happens mostly in components via finding today's data,
                // but we update the flag here.
                localStorage.setItem('palante_last_reset_date', today);
    ...
    }, [activeFocuses, user?.quoteIntensity, user?.contentTypePreference, notificationSettings.enabled]);

    useEffect(() => {
        console.log('[useAppProcess] 🔔 Notification sync check at', new Date().toISOString());
        ...
        if (configKey !== lastNotificationConfigRef.current) {
            console.log('[useAppProcess] 🔔 Config changed, rescheduling notifications');
    ...
    useEffect(() => {
        if (user && user.hapticsEnabled !== undefined) {
            localStorage.setItem('palante_haptics_enabled', String(user.hapticsEnabled));
        }
    }, [user?.hapticsEnabled]);

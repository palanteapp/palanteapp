...
import type { UserProfile, Quote, FavoriteQuote, Goal, DailyFocus, JournalEntry, ActivityLog, ActivityType, Milestone, MeditationReflection, NoiseEntry, DailyMorningPractice, RoutineStack, ContentType, QuoteSource } from './types';
...
import {
  Home, Wind, TrendingUp, BookOpen, User as UserIcon, Sun, Moon, Flower,
  BookMarked, Music, MessageCircle, Bell, ChevronDown, Check, Utensils,
  X, Target, Sparkles, ChevronRight, ChevronLeft, Waves, Mic, Compass, Heart,
  CheckCircle2, Layers, ArrowRight, Timer
} from 'lucide-react';
...
import { initializePracticeData, logPractice, checkMilestone, migrateStreakToPractice } from './utils/practiceUtils';
...
import { getTodayDate, getDaysDifference } from './utils/streakUtils';
...
function AppContent() {
  console.log('[APP] 🔄 Rendering AppContent at', new Date().toISOString());
  ...
  const [isGoalsExpanded, setIsGoalsExpanded] = useState(() => {
    const saved = localStorage.getItem('palante_goals_expanded');
    const isFirstVisit = saved === null;
    return isFirstVisit ? true : JSON.parse(saved);
  });
  ...
  const smartSuggestion = useSmartSuggestions(user, isDarkMode);
  ...
  const [showVibeCheck, setShowVibeCheck] = useState(() => !localStorage.getItem('palante_vibe_checked'));
  ...
  const refreshDailyQuote = useCallback((force = false) => {
    if (!user) return;
    const relevant = getRelevantQuotes(user);
    if (relevant.length > 0) {
      // Filter out current if forcing refresh
      const candidates = force && dailyQuote
        ? relevant.filter(q => q.id !== dailyQuote.id)
        : relevant;

      const randomQuote = candidates[Math.floor(Math.random() * candidates.length)];
      setDailyQuote(randomQuote);
      localStorage.setItem('palante_daily_quote', JSON.stringify(randomQuote));
      localStorage.setItem('palante_quote_date', new Date().toISOString().split('T')[0]);
  ...
  // Evening Wind-Down: Automatically darken theme after 9 PM
  useEffect(() => {
    if (shouldShowEveningMode && !isDarkMode) {
      setIsDarkMode(true); // Auto-enable dark mode for evening
    }
  }, [shouldShowEveningMode, isDarkMode]);
  ...
  const [skippedSections, setSkippedSections] = useState<string[]>(() => {
    // Reset skips at midnight is handled by the checkMidnight effect clearing simple storage, 
    // but here we just read the current day's skip list.
    const today = new Date().toISOString().split('T')[0];
    const saved = localStorage.getItem(`palante_skipped_${today}`);
  ...
  const [showIntroSequence, setShowIntroSequence] = useState(() => !localStorage.getItem('palante_intro_seen'));
  ...
  const handleIntroComplete = async (userData: {
    ...
    localStorage.setItem('palante_intro_seen', 'true');
    // 2. Mark vibe as checked (so the legacy modal doesn't pop up)
    localStorage.setItem('palante_vibe_checked', 'true');
    ...
    if (user) {
      const updatedUser = {
        ...
        quoteIntensity: userData.quoteIntensity as any,
  ...
  const [showDisclaimer, setShowDisclaimer] = useState(() => {
    if (!localStorage.getItem('palante_intro_seen')) return false; // Don't show if Intro is showing
  ...
  const handleAcknowledgeMissedDay = () => {
    // User acknowledges they forgot - no action needed, just close
    haptics.light();
    setShowRestDayModal(false);
  };
  ...
  useEffect(() => {
    ...
    const letterShownToday = sessionStorage.getItem('palante_letter_shown_today');
    ...
    if (hasLowEnergy && !letterShownToday) {
      ...
      if (undeliveredLetters.length > 0) {
        ...
        sessionStorage.setItem('palante_letter_shown_today', 'true');
      }
    }
  }, [user, showLetterRead]);
  ...
  const loadNewQuote = useCallback(async (userProfile: UserProfile, sourceOverride?: QuoteSource) => {
    ...
    // Safety Fallback: Ensure we ALWAYS have a quote
    let selectedQuote = quotes.length > 0 ? quotes[0] : null;

    if (!selectedQuote) {
      console.warn('No relevant quotes found, using emergency fallback');
      selectedQuote = {
    ...
    localStorage.setItem('palante_last_quote', JSON.stringify(selectedQuote));
  ...
  const { permission, settings: notificationSettings, updateNudgeConfig } = notifications;
  ...
  useEffect(() => {
    if (user && !userLoading) {
      ...
      const lastHour = parseInt(sessionStorage.getItem('palante_last_hour') || '-1');
      ...
      if (isTimeBoundary) {
        console.log('[App] ⏳ Time boundary crossed, refreshing quote...');
        loadNewQuote(user);
        sessionStorage.setItem('palante_last_hour', currentHour.toString());
      } else if (lastHour === -1) {
        sessionStorage.setItem('palante_last_hour', currentHour.toString());
      }
    }
  }, [hour, user, userLoading]);
  ...
  useEffect(() => {
    ...
    const checkedToday = sessionStorage.getItem('palante_rest_day_checked');
    ...
    // If exactly 1 day missed (yesterday), show rest day prompt
    if (daysSince === 1) {
    ...
    sessionStorage.setItem('palante_rest_day_checked', 'true');
  }, [user]);
  ...
  const handleWelcomeComplete = (userData: { name: string; profession: string; focusGoal: string; interests: string; gender?: string; ageRange?: string; tier: number; contentType: 'affirmations' | 'quotes' | 'mix'; sourcePreference: 'human' | 'ai' | 'mix' }) => {
    // Mark onboarding as completed
    localStorage.setItem('palante_onboarding_completed', 'true');
    ...
    if (user) {
      const updatedUser: UserProfile = {
        ...user,
        name: userData.name,
        quoteIntensity: userData.tier as any, // Legacy mapping from onboarding callback
        contentTypePreference: userData.contentType,
        sourcePreference: userData.sourcePreference
      };
  ...
  const handleToggleFavorite = async () => {
    ...
    console.log('Toggling favorite for quote:', quoteIdStr);
    console.log('Current user favorites:', user.favoriteQuotes);
  ...
  const handlePracticeUpdate = (practiceType: string) => {
    if (!user) return;
    ...
    if (milestone && isNew) {
      ...
      const milestoneMap: Record<string, 'week' | 'month' | 'century' | 'year'> = {
        'practices_7': 'week',
        'practices_30': 'month',
  ...
  useEffect(() => {
    const bootstrapWidget = async () => {
      console.log('[App] 🚀 Bootstrapping Widget Data...');
      if (Capacitor.getPlatform() === 'ios') {
        try {
          ...
          console.log('[App] ✅ Bootstrapped widget with initial state');
        } catch (e) {
  ...
  const navClass = isDarkMode
    ? 'bg-warm-gray-green/60 border-white/5 shadow-2xl backdrop-blur-2xl'
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-full backdrop-blur-md border transition-all duration-300 hover:scale-110 ${activeTab === 'coach'
              ? isDarkMode ? 'bg-pale-gold border-pale-gold text-warm-gray-green' : 'bg-sage border-sage text-white'
              : headerBtnClass
              } `}
          ...
          <PageTransition>
            <div className="min-h-screen px-8 pb-8 max-w-md mx-auto">
              ...
              <div className="w-full mb-6 animate-fade-in-slow text-center">
                ...
                {shouldShowEveningMode && (
                  <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-sage/20 to-pale-gold/20 border border-sage/30 backdrop-blur-sm animate-fade-in">
                    ...
                    <p className={`text-sm ${isDarkMode ? 'text-white/70' : 'text-warm-gray-green/70'}`}>
                    ...
                    {displayOrder.map((sectionId) => (
                      <div key={sectionId} id={`${sectionId}-section`} className="relative">
                        ...
                        {sectionId === 'todays_goals' && (
                          <div className={`p-6 rounded-2xl border mb-6 transition-all duration-500 ease-in-out ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/60 border-sage/20'} `}>
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => {
                                    ...
                                    localStorage.setItem('palante_goals_expanded', JSON.stringify(newState));
                                    haptics.light();
                                  }}
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button
                                      ...
                                      className={`px-4 rounded-xl flex items-center justify-center transition-all ${!newFocusText.trim()
                                        ...
                                        : isDarkMode
                                          ? 'bg-pale-gold text-rich-black hover:bg-pale-gold/90'
                        ...
                        {sectionId === 'accountability_coach' && (
                          <div
                            className={`mb-6 p-4 rounded-3xl flex items-center justify-between cursor-pointer transition-all ${isDarkMode
              ...
              <div className="min-h-screen px-8 pb-8 max-w-md mx-auto">
                <Momentum
                  ...
                  onUpdateUser={async (updatedUser) => {
                    console.log('[App] 🎯 Calling updateProfile for goal toggle');
                    await updateProfile(updatedUser);
                  }}
                  isDarkMode={isDarkMode}
                ...
                <Suspense fallback={
                  ...
                  <PomodoroTimer
                    ...
                    onAddHydration={() => {
                      ...
                      const savedHydration = localStorage.getItem('palante_fasting_hydration');
                      const current = savedHydration ? parseInt(savedHydration) : 0;
                      localStorage.setItem('palante_fasting_hydration', (current + 1).toString());
              ...
              <div className="min-h-screen max-w-md mx-auto h-full">
                <JapaneseWisdomView
                  ...
                  onStartFocus={(minutes: number, objective?: string) => {
                    ...
                    const focusRoutine: any = {
                      id: `focus-${Date.now()}`,
                      title: objective || (minutes === 1 ? 'Kaizen Focus' : 'Anchored Focus'),
                      name: objective || (minutes === 1 ? 'Kaizen' : 'Focus'),
                      steps: [
                        {
                          type: 'focus',
                          duration: minutes * 60,
        ...
        {showReturnToWisdom && activeTab !== 'wisdom' && activeTab !== 'coach' && (
          ...
          >
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-full shadow-2xl font-display font-medium transition-all active:scale-95 ${isDarkMode ? 'bg-pale-gold text-warm-gray-green' : 'bg-sage text-white'}`}
            >
              <ChevronLeft size={18} />
              Return to Wisdom
            </button>
      ...
      <AnimatePresence>
        {showToast && (
          ...
          >
            <div className={`px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 backdrop-blur-xl border ${isDarkMode ? 'bg-pale-gold/90 text-warm-gray-green border-white/20' : 'bg-sage/90 text-white border-sage/20'}`}>
      ...
      <Suspense fallback={null}>
        {
          user && (
            <MorningPractice
              ...
              onUpdateUser={(updates: any) => handleProfileUpdate((prev: UserProfile | null) => {
                if (!prev) return prev!;
                return { ...prev, ...updates };
              })}
            />

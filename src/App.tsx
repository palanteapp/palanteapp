          ...
          <div className="min-h-screen px-6 pb-8 animate-fade-in max-w-5xl mx-auto">
                    ...
                    {user.dailyFocuses.slice(0, 4).map(focus => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setUser(updatedUser);
                          api.updateUserProfile(user.id, updatedUser);
                        }}
                        className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all duration-200 ${focus.isCompleted
                          ? isDarkMode
                            ? 'bg-pale-gold/20 border-pale-gold text-pale-gold'
                            : 'bg-sage/20 border-sage text-sage'
                          : isDarkMode
                            ? 'bg-white/5 border-white/10 hover:bg-white/10'
                            : 'bg-white/60 border-sage/20 hover:bg-white'
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${focus.isCompleted
                            ...
                            {focus.isCompleted && (
                              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                                <path d="M2 6L5 9L10 3" stroke={isDarkMode ? "#6F7B6D" : "white"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                              </svg>
                            )}
                          </div>
                          <span className={`text-sm font-medium ${focus.isCompleted ? 'line-through opacity-60' : ''}`}>
                            {focus.text}
                          </span>
                        </div>
                      </button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-24">
              <button
                ...
                className={`p-4 rounded-2xl border transition-all hover:scale-105 ${isDarkMode
                  ? 'bg-white/5 border-white/10 hover:bg-white/10'
                  : 'bg-white/60 border-sage/20 hover:bg-white hover:shadow-spa'
                  }`}
              >
                <div className="flex flex-col items-center">
                  <Wind size={24} className={isDarkMode ? 'text-white/60' : 'text-sage/60'} />
                  <p className={`text-xs font-medium mt-2 ${isDarkMode ? 'text-white' : 'text-warm-gray-green'}`}>Breathe</p>
                </div>
              </button>
              <button

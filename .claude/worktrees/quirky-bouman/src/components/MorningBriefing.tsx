...
import type { Quote, JournalEntry } from '../types';
...
interface MorningBriefingProps {
    ...
    onDismiss: () => void;
    username: string;
    lastReflection?: JournalEntry | null;
}

type Step = 'greeting' | 'wisdom' | 'insight' | 'intent' | 'breath' | 'complete';

export const MorningBriefing: React.FC<MorningBriefingProps> = ({ quote, onComplete, username, lastReflection }) => {
    ...
    const handleNext = () => {
        if (step === 'wisdom') {
            // If we have a meaningful learning from yesterday, show it
            if (lastReflection?.midpoint) {
                setStep('insight');
            } else {
                setStep('intent');
            }
        }
        else if (step === 'insight') setStep('intent');
    ...
    useEffect(() => {
        if (step === 'breath') {
            ...
            Promise.resolve().then(() => setBreathPhase('inhale'));
    ...
    return (
        <div className={`fixed inset-0 z-50 flex flex-col items-center justify-center bg-sage-mid text-white overflow-y-auto transition-opacity duration-1000 ${isExiting ? 'opacity-0' : 'opacity-100'}`}>
            {/* Background Ambience */}
            ...
            <div className="relative z-10 max-w-lg w-full px-8 text-center">
                ...
                {step === 'complete' ? (
                    <div className="animate-fade-in-up">
                        ...
                        <button
                            onClick={handleExit}
                            className="mt-8 px-8 py-3 bg-pale-gold text-sage-dark rounded-full font-medium shadow-spa hover:bg-white transition-all"
                        >
                            Enter Palante
                        </button>
                    </div>
                    ...
                    <>
                        ...
                        {/* Step 2.5: Insight (Yesterday's Learning) */}
                        {step === 'insight' && lastReflection && (
                            <div className="animate-fade-in">
                                <div className="mb-8">
                                    <BookOpen size={32} className="text-blue-300 mx-auto mb-4" />
                                    <h3 className="text-sm font-bold tracking-widest uppercase text-white/40 mb-2">From Yesterday</h3>
                                    <h2 className="text-2xl font-light text-white/80 mb-2">You learned that...</h2>
                                </div>
                                <div className="bg-white/10 p-8 rounded-2xl backdrop-blur-md border border-white/10 mb-10 transform rotate-1">
                                    <p className="text-3xl font-display font-medium leading-relaxed text-blue-100">
                                        "{lastReflection.midpoint}"
                                    </p>
                                </div>
                                <button
                                    onClick={handleNext}
                                    className="group flex items-center gap-3 px-8 py-4 bg-white/10 hover:bg-white/20 rounded-full mx-auto transition-all backdrop-blur-md border border-white/10"
                                >
                                    <span className="font-medium">Apply This Today</span>
                                    <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                                </button>
                            </div>
                        )}
                                ...
                                <div className="flex justify-center gap-4">
                                    <button
                                        ...
                                        className={`group flex items-center gap-3 px-8 py-4 rounded-full transition-all ${focus.trim()
                                            ? 'bg-pale-gold text-sage-dark shadow-spa hover:bg-white'
                                            : 'bg-white/5 text-white/20 cursor-not-allowed'
                                            }`}
                                    >
                                        <span className="font-medium">Set Your Intention</span>
                                        <ArrowRight size={20} />
                                    </button>
                                </div>
                            </div>

...
export interface UserContext {
    ...
    /** Full structured memory brief from coachMemory.ts — injected into system prompt */
    userMemory?: string;
}
...
export const chatWithCoach = async (
    ...
    - Memory: You have rich context on their recent wins, challenges, meditations, goals, and energy. Reference it naturally if relevant (e.g., "Last week you mentioned [X]..." or "I noticed your energy's been [Y] lately..."). Never read the memory block back verbatim — weave it in organically.
...
STYLE: ${intensityDesc}
MEMORY USAGE: Reference their history naturally when relevant — never recite it. Use it to ask better questions and give more precise guidance.
...
/**
 * Generate a personalized daily morning brief (30-second read).
 * Called once per morning; result cached by DailyBriefCard.
 */
...
export const generatePersonalizedDailyBrief = async (
    userName: string,
    coachName: string | undefined,
    ctx: DailyBriefContext
): Promise<string> => {
    if (!GEMINI_API_KEY || GEMINI_API_KEY.includes('placeholder')) {
        return getFallbackDailyBrief(userName, ctx);
    }

    const coach = coachName ? `Coach ${coachName}` : 'Palante Coach';
    const hour = new Date().getHours();
    const timeLabel = hour < 12 ? 'morning' : hour < 18 ? 'afternoon' : 'evening';

    const prompt = `You are ${coach}, a high-performance wellness companion.
Generate a brief, personalized daily message for ${userName} to read at the start of their ${timeLabel}.
This is their private, AI-produced daily brief — like a personal producer who read everything they wrote.

USER DATA:
- Streak: ${ctx.streak} days
- Average energy (last week): ${ctx.averageEnergy !== null ? `${ctx.averageEnergy}/5` : 'Not logged'}
- Recent wins: ${ctx.recentWins.length ? ctx.recentWins.join(' | ') : 'None yet'}
- Recent challenges: ${ctx.recentChallenges.length ? ctx.recentChallenges.join(' | ') : 'None noted'}
- Active goals: ${ctx.activeGoals.length ? ctx.activeGoals.join(', ') : 'No active goals'}
- Yesterday's intention: ${ctx.latestIntention ?? 'Not set'}

REQUIREMENTS:
1. 3 sentences maximum. This must be readable in under 30 seconds.
2. Weave in 1-2 specific data points naturally — don't list them.
3. End with ONE clear, actionable micro-focus for today.
4. Tone: warm, direct, energizing. Never cheesy. No emojis.
5. Address them by first name once at the start.
6. NEVER use generic motivational filler like "You've got this!" or "Keep going!".

MEDICAL SAFETY: Never give medical, dietary, or clinical advice.

Generate the brief now (plain text, no formatting):`;
    ...
    try {
        ...
        if (!response.ok) return getFallbackDailyBrief(userName, ctx);

        const data = await response.json();
        const text = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
        return text || getFallbackDailyBrief(userName, ctx);
    } catch {
        return getFallbackDailyBrief(userName, ctx);
    }
};

const getFallbackDailyBrief = (userName: string, ctx: DailyBriefContext): string => {
    const firstName = userName.split(' ')[0];
    const streakLine = ctx.streak > 1 ? ` Your ${ctx.streak}-day streak is real momentum.` : '';
    const goalLine = ctx.activeGoals.length
        ? ` Today, keep your focus on: ${ctx.activeGoals[0]}.`
        : ' Pick one thing to move forward today.';
    const winLine = ctx.recentWins.length
        ? ` Yesterday you carried "${ctx.recentWins[0]}" — bring that energy.`
        : '';
    return `${firstName}, you showed up.${streakLine}${winLine}${goalLine}`;
};

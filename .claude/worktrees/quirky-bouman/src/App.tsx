...
function AppContent() {
  ...
  // Sync widget data whenever user or currentQuote changes
  useEffect(() => {
    if (!user) return;
    WidgetDataSync.syncAll(
      user,
      currentQuote ? { text: currentQuote.text, author: currentQuote.author } : undefined
    ).catch(e => console.error('[App] Widget sync failed:', e));
  }, [user, currentQuote]);
          ...
          <PageTransition>
            <div className="min-h-screen px-8 pb-8 max-w-md mx-auto">
              ...
              <div className="w-full mb-6 animate-fade-in-slow text-center">
                ...
                {/* Daily Brief Card — AI morning brief, shown once per day */}
                {user && (
                  <DailyBriefCard
                    user={user}
  ...
  if (showIntroSequence) {
    return (
      <ErrorBoundary name="CinematicIntro" onReset={() => setShowIntroSequence(false)}>

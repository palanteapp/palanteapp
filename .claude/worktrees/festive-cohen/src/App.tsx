...
function AppContent() {
  ...
  const gardenInView = useInView(gardenRef, { once: true, margin: '-60px 0px' });
  ...
  useEffect(() => {
    ...
    const scheduledDue = user.futureLetters.filter(l =>
      !l.hasBeenDelivered &&
      l.scheduledDeliveryDate &&
      new Date(l.scheduledDeliveryDate) <= today
    );

    if (scheduledDue.length > 0) {
      const letter = scheduledDue.sort((a, b) =>
        new Date(a.scheduledDeliveryDate!).getTime() - new Date(b.scheduledDeliveryDate!).getTime()
      )[0];

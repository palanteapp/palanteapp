...
export const Breathing = memo<BreathworkProps>(({ onComplete, onShowTip, isDarkMode = false }) => {
    ...
    const animate = useCallback(function tick(time: number) {
        ...
        if (activePhaseIndex !== phaseIndexRef.current) {
            ...
            if (enhancements.immersiveHaptics) {
                // SUB-MICRO HAPTICS: Medium pulses during inhale/exhale every 500ms
            ...
            if (enhancements.hapticDarkMode) {
                ...
                // Inhale = light (building), Exhale = selection (releasing)
                if (phase.name === 'Inhale' || phase.name === 'Exhale') {
                    if (Math.floor(timeInCurrentPhase / 1000) !== Math.floor((timeInCurrentPhase - 16) / 1000)) {
                        triggerHapticRef.current(phase.name === 'Inhale' ? 'light' : 'selection');
                    }
                }
            }
        }
            ...
            {enhancements.hapticDarkMode && status === 'active' && (
                ...
                >
                    <div className="text-center">
                        <p className="text-white/20 text-[9px] uppercase tracking-[0.35em] mb-12">
                            {activeTechnique === 'Box' ? 'Box Breathing' : activeTechnique === '4-7-8' ? '4-7-8 Technique' : 'Coherent Breathing'}
                        </p>
                        <div className="text-white text-6xl font-display font-light tracking-[0.15em] mb-4">
                            {isPaused ? 'PAUSED' : phaseName}
                        </div>
                        <div className="text-white/30 text-5xl font-mono font-light tabular-nums">
                            {timeLeftInPhase}s
                        </div>
                        <div className="text-white/15 font-mono text-sm mt-10">
                            {Math.floor(totalSecondsLeft / 60)}:{Math.floor(totalSecondsLeft % 60).toString().padStart(2, '0')} remaining
                        </div>
                    </div>
                    <div className="absolute bottom-14 text-center">
                        <p className="text-white/15 text-[9px] uppercase tracking-[0.3em]">
                            Double-tap to exit
                        </p>
                    </div>
                </div>

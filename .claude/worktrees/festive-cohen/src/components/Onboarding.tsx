...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete, isDarkMode, onToggleTheme }) => {
    ...
    if (step === 6) {
        ...
        return (
            <div className={`min-h-screen flex flex-col items-center pt-12 pb-6 px-6 transition-colors duration-500 relative overflow-hidden ${bgClass}`}>
                <ThemeToggle onToggleTheme={onToggleTheme} isDarkMode={isDarkMode} />

                {/* Background glow */}
                <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-96 h-64 rounded-full blur-[120px] opacity-20 pointer-events-none ${isDarkMode ? 'bg-pale-gold' : 'bg-terracotta-500'}`} />

                <div className="w-full max-w-lg animate-fade-in text-center flex flex-col h-full z-10">

                    {/* Icon */}
                    <div className="flex justify-center mb-6">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-pale-gold/20' : 'bg-terracotta-500/10'}`}>
                            <Heart size={36} className={isDarkMode ? 'text-pale-gold' : 'text-terracotta-500'} fill="currentColor" />
                        </div>
                    </div>
                    ...
                    <p className={`text-sm mb-1 ${textSecondary}`}>
                        Delivered on <span className={`font-bold ${isDarkMode ? 'text-pale-gold' : 'text-terracotta-500'}`}>{deliveryPreview}</span>
                    </p>
                    <p className={`text-xs mb-6 max-w-xs mx-auto ${textSecondary} opacity-60`}>
                    ...
                    <textarea
                        ...
                        rows={7}
                        className={`w-full p-5 rounded-2xl font-display resize-none focus:outline-none focus:ring-2 transition-all text-left ${isDarkMode
                            ? 'bg-white/5 border border-white/10 text-white placeholder-white/30 focus:ring-pale-gold/40'
                            : 'bg-white/80 border border-sage/20 text-sage-dark placeholder-sage-dark/30 focus:ring-terracotta-500/30'
                            }`}
                    />

                    <p className={`text-right text-xs mt-1 mb-6 ${textSecondary} opacity-40`}>
                        {onboardingLetter.length} characters
                    </p>
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                        <button
                            ...
                            disabled={onboardingLetter.trim().length === 0}
                            className={`w-full py-4 rounded-full font-bold text-base transition-all flex items-center justify-center gap-2 ${onboardingLetter.trim().length > 0
                                ? 'bg-terracotta-500 text-white shadow-xl hover:scale-105 hover:shadow-2xl'
                                : isDarkMode
                                    ? 'bg-white/10 text-white/40 cursor-not-allowed'
                                    : 'bg-sage/20 text-sage/40 cursor-not-allowed'
                                }`}
                        >
                            <Heart size={18} className={onboardingLetter.trim().length > 0 ? 'fill-current' : ''} />
                            Seal the Letter & Begin
                        </button>

                        <button
                            onClick={handleSubmit}
                            className={`text-xs uppercase tracking-widest font-bold hover:underline ${textSecondary} opacity-50`}
                        >
                            Skip — Begin Journey
                        </button>
                    </div>

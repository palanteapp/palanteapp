...
export const TrendChart: React.FC<TrendChartProps> = ({ datasets, isDarkMode }) => {
    ...
    const containerClasses = isFullscreen
        ? `fixed inset-0 z-50 flex flex-col items-center justify-center p-8 animate-fade-in ${isDarkMode ? 'bg-sage-mid' : 'bg-ivory'}`
    ...
    return (
        ...
        >
            ...
            {isFullscreen && (
                <div className="absolute top-12 left-8 md:left-12">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <h2 className={`text-4xl font-display font-medium ${isDarkMode ? 'text-white' : 'text-sage-dark'}`}>Momentum</h2>
                    </div>
                    <p className={`text-sm font-body font-medium uppercase tracking-widest ${isDarkMode ? 'text-white/50' : 'text-sage-dark/50'}`}>7 Day Trend</p>
                </div>
            )}
            ...
            <div className={`absolute bottom-4 left-6 flex gap-4 ${isFullscreen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                {datasets.map(d => (
                    <div key={d.label} className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-widest ${isDarkMode ? 'text-white/60' : 'text-sage-dark/60'}`}>{d.label}</span>
                    </div>
                ))}
            </div>
            ...
            {/* Labels */}
            {!isFullscreen && (
                <div className={`absolute top-4 right-6 text-xs font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity ${isDarkMode ? 'text-white/60' : 'text-sage-dark/40'}`}>
                    Tap to Expand
                </div>
            )}
        </div>

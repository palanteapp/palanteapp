...
export const DashboardQuoteCard: React.FC<DashboardQuoteCardProps> = ({
    ...
    // Gyroscope parallax — device tilt moves background and content in opposite layers
    ...
    useEffect(() => {
        const handleOrientation = (e: DeviceOrientationEvent) => {
            ...
            // beta:  front/back tilt [-180, 180], natural hold ≈ 45° → y axis
            ...
            rafRef.current = requestAnimationFrame(() => {
                // Background layer: moves WITH tilt (foreground feel)
                if (bgRef.current) {
                    bgRef.current.style.transform = `translate(${x * 10}px, ${y * 8}px) scale(1.06)`;
                }
                // Quote box: moves AGAINST tilt slightly (depth illusion)
                if (quoteBoxRef.current) {
                    quoteBoxRef.current.style.transform = `translate(${x * -5}px, ${y * -4}px)`;
                }
            });
        };
    ...
    const handleShare = async () => {
        ...
        try {
            // Target the already-rendered, visible preview in the ShareModal
            // (more reliable than capturing off-screen elements on iOS WebView)
            const element = document.getElementById('share-preview-container');
            if (!element) throw new Error('Preview element not found');

            // Scale up the small preview to a shareable resolution (~1600×2844)
            ...
            // Write image to device filesystem
            ...
            // Share via native share sheet — `files` sends the actual image
            // (using `files` instead of `url` is required for image sharing on iOS)
            ...
            // Graceful fallback: text-only share
            try {
                const { Share } = await import('@capacitor/share');
                await Share.share({
                    title: 'Inspiration from Palante',
            ...
            } catch { /* ignore — share sheet cancelled */ }
        } finally {
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div style={{
                        ...
                        {/* ── Background SVG art — gyroscope layer ── */}
                        <div
                            ref={bgRef}
                            style={{
                        ...
                        {/* ── Inner White Quote Box — counter-parallax layer ── */}
                        <div
                            ref={quoteBoxRef}
                            style={{
                                position: 'relative',

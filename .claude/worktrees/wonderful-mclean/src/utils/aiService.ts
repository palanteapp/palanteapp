...
export interface UserContext {
    ...
    activeFocus?: string;
    coachName?: string;
    // Memory fields (legacy — kept for compatibility)
    ...
    // Full memory brief built by buildCoachMemoryContext()
    userMemory?: string;
}
...
export const chatWithCoach = async (
    ...
    const coachIdentity = context.coachName ? `Coach ${context.coachName}` : 'Palante Coach';
    const memoryBlock = context.userMemory ?? '';

    // Construct System Prompt
    const systemPrompt = `You are ${coachIdentity}, a warm, nurturing, and deeply supportive friend and mentor.
...
- Memory: You have the full memory brief above. Reference it naturally when it's relevant — mention specific wins, challenges, intentions, or patterns by name. Sound like someone who actually remembers them (e.g., "You wrote last Thursday that anxiety before meetings was high — how did that go?").
- Conversation Style: Natural back-and-forth. Keep responses concise at first. Listen more than you talk.
- App Guidance: Only suggest ONE relevant app feature if it feels truly helpful.

GOAL:
Build genuine connection. Use their history to provide specific, personal guidance. Only transition to active coaching once you've truly listened.
...
export const chatWithCoachPillar = async (
    ...
    const intensityDesc = getIntensityDescription(context.quoteIntensity);
    const timeOfDay = new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 18 ? 'afternoon' : 'evening';
    const coachIdentityPillar = context.coachName ? `Coach ${context.coachName}` : 'Palante Coach';
    const memoryBlockPillar = context.userMemory ?? '';

    const systemPrompt = `${pillarPrompt}

You are responding as ${coachIdentityPillar}.
...
MEMORY INSTRUCTION: Use the memory brief above to make your responses specific and personal. Reference their real data — journal entries, goals, energy patterns, morning intentions — when relevant to this ${pillar} conversation. Sound like someone who has been paying attention.

STYLE: ${intensityDesc}
RESPONSE LENGTH: Focused and conversational. Under 120 words unless the user asks for something detailed.

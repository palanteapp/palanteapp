...
class CrossfadingSound {
    ...
    async play(startVol = 0.5) {
        if (this.isStarting) return; // Guard: prevent overlapping play() calls
        this.isStarting = true;
        this.volume = startVol;
        this.init();
        const ctx = getAudioContext();
        if (!this.audio1 || !this.audio2 || !this.gainNode1 || !this.gainNode2 || !ctx) {
...
export const SoundMixer: React.FC<SoundMixerProps> = ({ isDarkMode: _isDarkMode, isVisible, onClose, user, onSaveMix, onDeleteMix, source }) => {
    ...
    // MediaSession — lock screen Now Playing panel
    useEffect(() => {
        ...
        // Build a human-readable title from active sound labels
        const activeLabels = Array.from(activeSounds)
            .map(id => SOUNDS.find(s => s.id === id)?.label ?? id)
            .join(' · ');
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            album: 'Soundscapes',
            artwork: [
                { src: '/logo-gold.png', sizes: '512x512', type: 'image/png' },
            ],
        });

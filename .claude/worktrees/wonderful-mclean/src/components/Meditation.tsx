...
export const Meditation = memo<MeditationProps>(({ isDarkMode, onComplete, onSaveReflection, tipsEnabled: _tipsEnabled = true, onShowTip: _onShowTip, onStrategize: _onStrategize, user: _user, onSaveMix: _onSaveMix, onDeleteMix: _onDeleteMix, onOpenSoundMixer }) => {
    ...
    const [selectedMantra, setSelectedMantra] = useState<Mantra>(() => MANTRAS[Math.floor(Math.random() * MANTRAS.length)]);
    ...
    const [_soundVolumes, setSoundVolumes] = useState<Record<string, number>>({});
    ...
    useEffect(() => {
        const handleSoundsChanged = (event: CustomEvent<{ activeSounds?: string[]; volumes?: Record<string, number> }>) => {
    ...
    const startReflectionDictation = () => {
        ...
        const SpeechRecognitionCtor = (window as Record<string, unknown>).SpeechRecognition || (window as Record<string, unknown>).webkitSpeechRecognition;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const recognition: any = new (SpeechRecognitionCtor as new () => unknown)();
                        ...
                        <div className="relative mb-6">
                            <textarea
                                ...
                                className={`w-full p-4 pr-12 rounded-xl h-32 outline-none resize-none ${isDarkMode
                                    ...
                                    : 'bg-white text-sage-dark placeholder-sage-dark/30 border border-sage/20 focus:border-sage'
                                    }`}
                                placeholder="I feel..."
                            />
                            <button
                    ...
                    <div className="space-y-6 animate-fade-in">
                        <input
                            ...
                            className={`w-full text-center bg-transparent border-b-2 text-2xl font-body focus:outline-none pb-2 transition-colors ${isDarkMode
                                ...
                                : 'border-sage/20 text-sage-dark placeholder-sage-dark/30 focus:border-sage'
                                }`}
                        />
                    </div>
                ) : (

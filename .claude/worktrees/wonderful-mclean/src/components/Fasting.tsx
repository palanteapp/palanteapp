...
export const Fasting = memo<FastingProps>(({ isDarkMode, onOpenCoach }) => {
    ...
    const [goalWeight, setGoalWeight] = useState<string>(() => {
        return localStorage.getItem(STORAGE_KEYS.FASTING_GOAL_WEIGHT) || '';
    });
    ...
    const [fastingHistory, setFastingHistory] = useState<Array<{ date: string, completed: boolean, targetHours: number, actualHours: number }>>(() => {
        const saved = localStorage.getItem(STORAGE_KEYS.FASTING_HISTORY);
        return saved ? JSON.parse(saved) : [];
    });

    // Fasting streak
    const [fastingStreak, setFastingStreak] = useState<{ current: number, longest: number }>(() => {
        const saved = localStorage.getItem(STORAGE_KEYS.FASTING_STREAK);
    ...
    useEffect(() => {
        ...
        localStorage.setItem(STORAGE_KEYS.FASTING_TARGET, targetDuration.toString());
    ...
    const handleEndFast = () => {
        ...
        localStorage.setItem(STORAGE_KEYS.FASTING_HISTORY, JSON.stringify(updatedHistory));
        ...
        if (completed) {
            ...
            localStorage.setItem(STORAGE_KEYS.FASTING_STREAK, JSON.stringify(newStreak));
        }
    ...
    const handleWeightSubmit = () => {
        if (currentWeight) {
            const weights = JSON.parse(localStorage.getItem(STORAGE_KEYS.FASTING_WEIGHTS) || '[]');
            const newWeight = parseFloat(currentWeight);
            weights.push({ date: new Date().toISOString(), weight: newWeight });
            localStorage.setItem(STORAGE_KEYS.FASTING_WEIGHTS, JSON.stringify(weights));
    ...
    // Science encouragement popup — rotates through fasting milestones
    ...
    const ringColor = isDarkMode ? '#E5D6A7' : '#E5D6A7'; // amber for both modes
    ...
    return (
        <div className="w-full flex flex-col items-center px-6 pt-6 pb-32 animate-fade-in">
            ...
            {/* Start/End Time — Inline editable cards while fasting is active */}
            {status === 'active' && startTime && (
                <div className="w-full max-w-sm mb-6 grid grid-cols-2 gap-3">
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (!isNaN(picked.getTime()) && picked <= new Date()) {
                                    setStartTime(picked.toISOString());
                                    haptics.selection();
                                }
                            }}
                            className="w-full bg-transparent outline-none text-xs font-medium text-pale-gold cursor-pointer"
                    ...
                    <div className={`p-3 rounded-xl ${bgClass}`}>
                        ...
                        <div className={`text-[9px] mt-1 opacity-60 ${textSecondary}`}>
                            {(() => {
                                const total = targetDuration * 3600;
                                const remaining = Math.max(0, total - elapsedSeconds);
                                const h = Math.floor(remaining / 3600);
                                const m = Math.floor((remaining % 3600) / 60);
                                return remaining > 0 ? `${h}h ${m}m left` : 'Target reached';
                            })()}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center z-10 px-6 text-center">
                    {status === 'active' ? (
                        ...
                        <>
                            ...
                            {showPresets && (
                                <div className="absolute top-full mt-2 w-48 py-2 rounded-xl shadow-xl backdrop-blur-xl z-50 flex flex-col gap-1 overflow-hidden animate-slide-in-up bg-[#3A1700]/80 border border-white/10">
                                    {PRESETS.map(p => (
                                        <button
                                            key={p.hours}
                                            onClick={() => {
            ...
            {/* Science Popup Trigger — gentle encouragement while fasting */}
            {status === 'active' && (
                <button
                    onClick={() => {
                        // Pick tip based on elapsed hours
                        let tipIdx = 0;
                        if (elapsedHours >= 24) tipIdx = 4;
                        else if (elapsedHours >= 18) tipIdx = 3;
                        else if (elapsedHours >= 12) tipIdx = 2;
                        else if (elapsedHours >= 8) tipIdx = 1;
                        else tipIdx = 0;
                        setCurrentScienceTip(tipIdx);
                        setShowSciencePopup(true);
                        haptics.light();
                    }}
                    className={`w-full max-w-sm mb-5 py-3 px-5 rounded-2xl flex items-center gap-3 transition-all active:scale-[0.98] border ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white/10 border-white/15 hover:bg-white/20'}`}
                >
                    <span className="text-xl">✨</span>
                    <div className="text-left flex-1">
                        <div className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-pale-gold' : 'text-pale-gold'}`}>
                            What's happening right now?
                        </div>
                        <div className={`text-xs mt-0.5 ${textSecondary}`}>
                            {currentStage.stage} · Tap for science + encouragement
                        </div>
                    </div>
                </button>
            )}
            ...
            <div className={`w-full max-w-sm p-5 rounded-2xl mb-6 transition-all ${bgClass}`}>
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`flex-[2] py-3 rounded-xl font-medium transition-all active:scale-95 ${isDarkMode ? 'bg-pale-gold text-sage-dark hover:bg-pale-gold/90' : 'bg-terracotta-500 text-white hover:bg-sage-600'}`}
                    >
                        Add Glass
                    </button>
                </div>
            ...
            {/* Start/End Actions — clean, simple datetime-local approach */}
            <div className="w-full max-w-sm flex flex-col gap-4 mb-6">
                {status === 'idle' && (
                    <div className={`p-4 rounded-2xl ${bgClass}`}>
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {manualStartTime && (
                                <button
                                    onClick={() => { setManualStartTime(null); haptics.light(); }}
                                    className={`text-[9px] font-bold uppercase tracking-wide opacity-50 hover:opacity-100 ${textSecondary}`}
                                >
                                    Reset to Now
                                </button>
                            )}
                        ...
                        <input
                            ...
                            value={
                                manualStartTime
                                    ? `${manualStartTime.getFullYear()}-${String(manualStartTime.getMonth()+1).padStart(2,'0')}-${String(manualStartTime.getDate()).padStart(2,'0')}T${manualStartTime.toTimeString().slice(0,5)}`
                            ...
                            onChange={(e) => {
                                ...
                                const picked = new Date(e.target.value);
                                if (!isNaN(picked.getTime()) && picked <= new Date()) {
                                    setManualStartTime(picked);
                                    haptics.selection();
                                }
                            }}
                            className="w-full px-4 py-3 rounded-xl border outline-none font-body text-sm transition-all bg-pale-gold/10 border-pale-gold/30 text-pale-gold focus:border-pale-gold"
                            style={{ colorScheme: 'dark' }}
                        />
                        <p className={`text-[10px] mt-2 opacity-50 ${textSecondary}`}>
                            {manualStartTime ? `Starting from ${formatDateTime(manualStartTime)}` : 'Will start from right now'}
                        </p>
                    </div>
                )}
                ...
                {status === 'idle' ? (
                    <button
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-lg tracking-wide transition-all hover:scale-[1.02] active:scale-95 shadow-lg flex items-center justify-center gap-2 ${'bg-terracotta-500 text-white hover:scale-105'}`}
                    >
                ...
                {status === 'active' && (
                    <div className="flex justify-center gap-3 animate-fade-in">
                        <span className={`text-[10px] font-bold uppercase tracking-wider opacity-40 self-center ${textSecondary}`}>Adjust start:</span>
                        {[-60, -15, 15].map(min => (
                            <button
                                key={min}
                                onClick={() => { adjustStartTime(min); }}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all active:scale-95 ${isDarkMode ? 'border-white/10 bg-white/5 hover:bg-white/10 text-white/60' : 'border-sage/10 bg-sage/5 hover:bg-sage/10 text-sage/60'}`}
                            >
                                {min > 0 ? `+${min}m` : min === -60 ? '-1h' : '-15m'}
                            </button>
                        ))}
                    </div>
                )}
            ...
            {(() => {
                ...
                const recentWeights = weights.slice(-7) as { weight: number; date: string }[];
            ...
            {showWeightLog && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="absolute inset-0 bg-[#3A1700]/30 backdrop-blur-md" onClick={() => setShowWeightLog(false)} />
                    <div className={`relative w-full max-w-sm p-6 rounded-3xl shadow-2xl ${isDarkMode ? 'bg-sage-mid/90 backdrop-blur-xl border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-sage/20'}`}>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 rounded-xl font-medium ${'bg-terracotta-500 text-white hover:scale-105'}`}
                            >
                                Save
                            </button>
                        </div>
            ...
            {showGoalWeightModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="absolute inset-0 bg-[#3A1700]/30 backdrop-blur-md" onClick={() => setShowGoalWeightModal(false)} />
                    <div className={`relative w-full max-w-sm p-6 rounded-3xl shadow-2xl ${isDarkMode ? 'bg-sage-mid/90 backdrop-blur-xl border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-sage/20'}`}>
                            ...
                            <button
                                onClick={() => {
                                    if (tempGoalWeight) {
                                        ...
                                        localStorage.setItem(STORAGE_KEYS.FASTING_GOAL_WEIGHT, tempGoalWeight);
            ...
            {showAutophagyModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="absolute inset-0 bg-[#3A1700]/30 backdrop-blur-md" onClick={() => setShowAutophagyModal(false)} />
                    <div className={`relative w-full max-w-sm p-6 rounded-3xl shadow-2xl ${isDarkMode ? 'bg-sage-mid/90 backdrop-blur-xl border border-white/10' : 'bg-white/90 backdrop-blur-xl border border-sage/20'}`}>
                        <div className="text-center">
                            ...
                            <button
                                ...
                                className={`w-full py-3 rounded-xl font-medium ${'bg-terracotta-500 text-white hover:scale-105'}`}
                            >
                                Amazing! 🎉
                            </button>
                        </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="mb-6 flex justify-center">
                            <input
                                ...
                                className={`px-4 py-3 rounded-xl font-medium text-center outline-none focus:ring-2 focus:ring-amber/50 ${isDarkMode
                        ...
                        <div className="flex gap-3 mt-8">
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 rounded-xl font-bold shadow-lg transition-transform active:scale-95 ${'bg-terracotta-500 text-white hover:scale-105'}`}
                            >
                                Save
                            </button>
                        </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-center">
                                    <input
                                        ...
                                        value={editStartDate}
                                        onChange={(e) => setEditStartDate(e.target.value)}
                                        className={`px-4 py-2 rounded-xl text-sm font-medium text-center outline-none focus:ring-2 focus:ring-amber/50 ${isDarkMode
        ...
        {/* Science & Encouragement Popup */}
        {showSciencePopup && (
            <div
                className="fixed inset-0 z-[200] flex items-end justify-center pb-10 px-6"
                style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)' }}
                onClick={() => setShowSciencePopup(false)}
            >
                <div
                    className="w-full max-w-sm rounded-[28px] overflow-hidden shadow-2xl animate-slide-in-up"
                    style={{ background: isDarkMode ? '#1a2318' : '#1a2318' }}
                    onClick={e => e.stopPropagation()}
                >
                    <div className="h-1.5 w-full bg-pale-gold" />
                    <div className="p-7">
                        <div className="flex items-center gap-3 mb-4">
                            <span className="text-3xl">{SCIENCE_TIPS[currentScienceTip].icon}</span>
                            <div>
                                <div className="text-[10px] font-bold uppercase tracking-widest text-pale-gold mb-0.5">
                                    {SCIENCE_TIPS[currentScienceTip].milestone}
                                </div>
                                <h3 className="text-lg font-display font-bold text-white">
                                    {SCIENCE_TIPS[currentScienceTip].headline}
                                </h3>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-white/10 pt-4 mb-5">
                            <p className="text-sm font-medium text-pale-gold italic">
                                "{SCIENCE_TIPS[currentScienceTip].encouragement}"
                            </p>
                        </div>
                        {/* Navigator dots */}
                        <div className="flex justify-center gap-2 mb-5">
                            {SCIENCE_TIPS.map((_, i) => (
                                <button
                                    key={i}
                                    onClick={() => setCurrentScienceTip(i)}
                                    className={`w-2 h-2 rounded-full transition-all ${i === currentScienceTip ? 'bg-pale-gold w-5' : 'bg-white/20'}`}
                                />
                            ))}
                        </div>
                        <button
                            onClick={() => setShowSciencePopup(false)}
                            className="w-full py-3.5 rounded-2xl font-bold text-xs uppercase tracking-widest bg-pale-gold text-sage-dark transition-all active:scale-95"
                        >

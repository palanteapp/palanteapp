...
describe('checkMilestone', () => {
    ...
    it('returns no milestone when truly under all thresholds', () => {
        const result = checkMilestone(0, freshMilestones);
        expect(result.milestone).toBeNull();
        expect(result.isNew).toBe(false);
    });

    it('detects the 3-practice milestone', () => {
        const result = checkMilestone(5, freshMilestones);
        expect(result.milestone).toBe('practices_3');
        expect(result.isNew).toBe(true);
    });

    it('detects the 7-practice milestone', () => {
        // Simulate a realistic state: user at 7 practices, 1 and 3 already celebrated
        const result = checkMilestone(7, { ...freshMilestones, practices_1: true, practices_3: true });
    ...
    it('does not re-trigger an already-reached milestone', () => {
        // Realistic state: user at 7 practices, all lower milestones already celebrated
